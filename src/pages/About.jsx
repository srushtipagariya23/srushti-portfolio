...
const About = () => {
  ...
  const handleCommand = (e) => {
    if (e.key === 'Enter' && input.trim()) {
      ...
      switch(cmd) {
        ...
        case 'projects': output = 'Inforens (UX Strategy), Scottish Widows (Financial Wellbeing), Sweet Lies (Inclusive Education),Bored Directors (Education).'; break;
  ...
  const metaphors = [
    { id: "01", title: "A Radio", text: "Because stories, music, and saying the thing that needs to be said have always found me.", color: "from-blue-500", icon: <span className="text-5xl drop-shadow-lg leading-none">📻</span> },
    { id: "02", title: "A Mirror", text: "Because I reflect, rethink, and rarely leave myself unquestioned.", color: "from-slate-400", icon: <span className="text-5xl drop-shadow-lg leading-none">🪞</span> },
    { id: "03", title: "Sun, Moon & Clouds", text: "Because one version of me was never going to be enough.", color: "from-yellow-400", icon: <span className="text-3xl md:text-4xl drop-shadow-lg tracking-widest leading-none">☀️🌙☁️</span> },
    { id: "04", title: "6°", text: "Because I like people and ideas that feel relatable, not distant.", color: "from-green-400", icon: <span className="text-5xl drop-shadow-lg leading-none">🤝</span> },
    { id: "05", title: "A Chilli", text: "Because I may not be for everyone, but the right people never forget me.", color: "from-red-500", icon: <span className="text-5xl drop-shadow-lg leading-none">🌶️</span> },
    { id: "06", title: "Spotify", text: "Because I want people to feel fully themselves around me.", color: "from-emerald-500", icon: <span className="text-5xl drop-shadow-lg leading-none">🎧</span> },
    { id: "07", title: "A Dog", text: "Because I love deeply, stay loyal, and show up fully.", color: "from-orange-400", icon: <span className="text-5xl drop-shadow-lg leading-none">🐕</span> },
    { id: "08", title: "Earrings", text: "Because small does not mean unnoticed.", color: "from-purple-400", icon: <span className="text-5xl drop-shadow-lg leading-none">✨</span> }
  ];
  ...
  return (
    <div 
      ref={containerRef} 
      className={`relative min-h-screen w-full overflow-hidden ${bgStyle} font-montserrat transition-colors duration-1000 selection:bg-brand-accent-blue selection:text-white md:cursor-none pb-0`}
      style={{ '--mouse-x': '50vw', '--mouse-y': '50vh', '--tilt-x': '0deg', '--tilt-y': '0deg' }}
    >
      
      {/* CUSTOM SUN CURSOR - Hidden on mobile */}
      <div 
        className="hidden md:block pointer-events-none fixed z-[99999] transition-transform duration-75 mix-blend-screen"
        style={{
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12 relative z-10">
        ...
        <section className="min-h-[80vh] flex flex-col justify-center mb-32 relative pt-12">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
            ...
            <div className="lg:col-span-7 transform-gpu md:style-[transform:rotateX(var(--tilt-x))_rotateY(var(--tilt-y))_transform-style-preserve-3d]">
              <h1 className={`font-poppins text-4xl md:text-7xl font-black uppercase tracking-tighter mb-6 flex flex-wrap ${theme === 'light' ? 'text-brand-blue' : 'text-white'}`}>
                <span className="flex">
                  {'Designing'.split('').map((char, i) => <span key={i} className="char-reveal block">{char}</span>)}
                </span>
                <span className="w-3 md:w-6"></span>
                <span className="flex">
              ...
              <div className={`backdrop-blur-xl ${glassStyle} p-6 md:p-8 rounded-2xl shadow-2xl relative overflow-hidden group`}>
                ...
                <p className={`text-base md:text-xl font-light leading-relaxed relative z-10 ${theme === 'light' ? 'text-slate-600' : 'text-slate-300'}`}>
        ...
        <section className="py-24 relative min-h-[90vh] flex flex-col items-center justify-center overflow-hidden scroll-fade-up border-y border-white/5">
          <div className="text-center mb-12 relative z-20">
            <h2 className={`font-poppins text-3xl md:text-5xl font-black uppercase tracking-widest mb-4 ${theme === 'light' ? 'text-brand-blue' : 'text-white'}`}>
            ...
            <p className={`font-montserrat text-sm md:text-base max-w-2xl mx-auto ${theme === 'light' ? 'text-slate-500' : 'text-slate-400'}`}>
              The people, places, plot twists, and questions that shaped how I see. <br className="hidden md:block"/> Click/Tap on a planet to explore.
            </p>
          </div>
          ...
          >
            <div className="absolute z-30 w-24 h-24 md:w-32 md:h-32 rounded-full bg-gradient-to-br from-brand-accent-blue to-purple-700 flex flex-col items-center justify-center shadow-[0_0_50px_rgba(124,58,237,0.6)] md:cursor-none group">
              <span className="font-poppins font-black text-white text-lg md:text-xl tracking-widest drop-shadow-md">SRUSHTI</span>
              <div className="absolute w-64 md:w-72 opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none bg-black/90 backdrop-blur-xl border border-white/10 p-4 md:p-5 rounded-xl -top-32 md:-top-40 left-1/2 -translate-x-1/2 shadow-2xl text-center z-50">
                 <p className="text-[9px] md:text-[10px] text-brand-accent-blue uppercase tracking-widest mb-3 font-bold leading-tight">A world built from stories, questions, and very good food.</p>
                 <p className="text-[10px] md:text-xs text-slate-300 leading-relaxed font-montserrat">I grew up in a joint family, where every person had a different opinion, story, and way of seeing things. I think that is where I started learning to listen.</p>
              </div>
            </div>
            ...
            {constellationData.map((planet, i) => (
              ...
              >
                 <div 
                   className={`absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 rounded-full flex items-center justify-center md:cursor-none hover:scale-125 transition-all duration-300 bg-gradient-to-br ${planet.color} shadow-lg shadow-${planet.color.split('-')[1]}/50 pointer-events-auto`}
                   style={{ 
                   ...
                   onClick={(e) => {
                    ...
                    if (finalLeft + cardWidth > ww) finalLeft = x - cardWidth - 20;
                    if (finalTop + cardHeight > wh) finalTop = y - cardHeight - 20;
          ...
          <div className="mt-16 text-center relative z-20">
             <h3 className={`font-poppins text-xl md:text-4xl font-light tracking-wide leading-snug ${theme === 'light' ? 'text-brand-blue' : 'text-slate-300'}`}>
               I collect stories.<br />
               <span className="font-bold text-brand-accent-blue">Then I design with what they reveal.</span>
             </h3>
          </div>
        </section>

        <section className="py-24 md:py-32 relative scroll-fade-up">
          <div className="text-center mb-16 md:mb-20">
            ...
            <p className={`font-montserrat text-sm md:text-base max-w-2xl mx-auto ${theme === 'light' ? 'text-slate-500' : 'text-slate-400'}`}>
              Tap/Hover over the artifacts to decode the pieces of my personality.
            </p>
          </div>
            ...
            {metaphors.map((item, i) => (
              ...
              >
                <div className="w-full h-full relative transition-all duration-[800ms] ease-[cubic-bezier(0.25,1,0.5,1)] group-hover:[transform:rotateY(180deg)] group-focus:[transform:rotateY(180deg)]" style={{ transformStyle: 'preserve-3d' }}>
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center p-6 md:p-8 rounded-2xl border border-brand-accent-blue/50 bg-[#0a0a0a] shadow-[0_0_30px_rgba(124,58,237,0.3)] overflow-hidden" style={{ backfaceVisibility: 'hidden', transform: 'rotateY(180deg)' }}>
                    ...
                    <p className="font-montserrat text-sm md:text-sm text-center leading-relaxed font-medium text-slate-200 relative z-10">"{item.text}"</p>
                    <div className="absolute bottom-5 right-5 font-mono text-[8px] text-brand-accent-blue tracking-widest uppercase font-bold">// Decoded</div>
                  </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 mb-16">
            <div className="grid grid-cols-2 gap-4 md:gap-6">
              {[
                ...
                <div key={i} className={`backdrop-blur-md ${glassStyle} p-4 md:p-6 rounded-xl flex flex-col justify-center items-center text-center group hover:border-brand-accent-blue transition-colors duration-300 md:cursor-none float-slow shadow-lg`} style={{ animationDelay: `${i * 0.2}s` }}>
                  <span className="stat-counter font-poppins text-3xl md:text-5xl font-black text-brand-accent-blue mb-2" data-target={stat.target} data-suffix={stat.suffix}>0</span>
                  <span className={`font-mono text-[9px] md:text-[10px] uppercase tracking-widest transition-colors ${theme === 'light' ? 'text-slate-500 group-hover:text-brand-blue' : 'text-slate-400 group-hover:text-white'}`}>{stat.label}</span>
                </div>
              ))}
            </div>

            <div className="bg-[#0A0A0A] border border-white/10 rounded-xl overflow-hidden shadow-2xl flex flex-col h-[300px] md:h-[350px]">
      ...
      <section ref={horizontalRef} className="relative w-full h-screen overflow-hidden flex items-center bg-brand-blue z-20 border-t border-white/10 mt-12">
        ...
        <div ref={scrollWrapperRef} className="flex items-center gap-12 md:gap-20 px-[15vw] h-full w-max">
          {memories.map((mem, i) => {
            ...
            const marginTop = i % 2 === 0 ? (i%3 === 0 ? '-mt-24 md:-mt-32' : 'mt-24 md:mt-40') : (i%4 === 0 ? '-mt-32 md:-mt-48' : 'mt-16 md:mt-24');
            const width = i % 3 === 0 ? 'w-40 md:w-64' : (i % 2 === 0 ? 'w-36 md:w-56' : 'w-32 md:w-48');
            ...
            return (
              <React.Fragment key={i}>
                ...
                {i === 7 && (
                  <div className="relative w-64 md:w-[600px] z-20 mx-6 md:mx-8">
                    <h3 className="font-poppins font-black text-3xl md:text-6xl text-white leading-[1.1] uppercase tracking-tighter mix-blend-difference">
                      "WHO AM I, <br/><span className="text-[#8244F5]">WITHOUT MY WIDE SMILE.</span>"
                    </h3>
                    <p className="font-montserrat text-slate-400 font-light mt-4 text-base md:text-2xl mix-blend-difference opacity-90">
      ...
      <section className="py-24 md:py-32 relative bg-[#050505] z-10">
        <div className="max-w-7xl mx-auto px-6 md:px-12">
          <div className="text-center mb-16 md:mb-24">
            <h2 className="font-poppins text-3xl md:text-6xl font-black text-white uppercase tracking-tighter mb-4">
              The <span className="text-brand-accent-blue">World</span> Through My Lens.
            </h2>
            <p className="font-montserrat text-slate-400 font-light max-w-2xl mx-auto text-sm md:text-base">
          ...
          <div className="columns-2 sm:columns-2 lg:columns-3 xl:columns-4 gap-4 md:gap-6 space-y-4 md:space-y-6">
            {naturePhotos.map((nat, i) => (
              <div key={i} className="nature-photo break-inside-avoid relative group overflow-hidden bg-white/5 p-1 md:p-2 border border-white/10 hover:border-brand-accent-blue/50 transition-colors duration-500 md:cursor-none rounded-sm">
      ...
      {activePlanet && (
         <div 
           className="fixed z-[9999] bg-[#0a0a0a]/95 backdrop-blur-2xl border border-brand-accent-blue/40 p-5 md:p-8 rounded-2xl max-w-xs md:max-w-sm w-[90%] shadow-[0_0_80px_rgba(124,58,237,0.6)] animate-in fade-in duration-200 md:cursor-none"
           style={modalStyles}
         >
            <button 
              onClick={() => setActivePlanet(null)} 
              className="absolute top-3 right-3 md:top-4 md:right-4 w-8 h-8 flex items-center justify-center rounded-full bg-white/5 hover:bg-white/10 text-slate-400 hover:text-white transition-colors border border-white/10"
            >
              ✕
            </button>
            <span className="text-[9px] md:text-[10px] font-mono uppercase tracking-widest text-brand-accent-blue mb-2 block font-bold">Orbit 0{activePlanet.id}: {activePlanet.name}</span>
            <h3 className="font-poppins text-xl md:text-2xl font-bold text-white mb-3 md:mb-4 leading-tight">{activePlanet.title}</h3>
            <p className="font-montserrat text-xs md:text-sm text-slate-300 leading-relaxed mb-4 md:mb-6 whitespace-pre-line">{activePlanet.story}</p>
            <div className="bg-brand-accent-blue/10 border-l-2 border-brand-accent-blue p-3 md:p-4 rounded-r-lg">
               <span className="text-[8px] md:text-[9px] font-mono uppercase tracking-widest text-brand-accent-blue font-bold block mb-1">What Stayed:</span>
               <p className="font-montserrat text-[10px] md:text-xs text-white font-medium leading-relaxed">{activePlanet.stayed}</p>
            </div>
         </div>
      )}
