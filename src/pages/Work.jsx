...
const Work = () => {
  ...
  return (
    <main ref={pageRef} className="w-full min-h-screen bg-[#F8F9FA] relative overflow-hidden selection:bg-brand-accent-blue selection:text-white">
      ...
      <div className="w-full block pointer-events-none" style={{ height: '120px', minHeight: '120px' }} aria-hidden="true"></div>
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12 pb-24 md:pb-32 relative z-10">
        
        <div className="mb-16 md:mb-32 border-b border-slate-200/80 pb-8 md:pb-12">
          ...
          <p className="work-header-anim font-montserrat text-base md:text-xl text-slate-500 font-light max-w-2xl leading-relaxed">
        ...
        <div className="space-y-24 md:space-y-48">
          {projects.map((project, index) => (
            <div key={project.id} className="project-row group flex flex-col md:flex-row gap-8 md:gap-20 items-center md:cursor-none">
              
              <Link to={`/case-study/${project.id}`} className={`project-image-container block md:cursor-none w-full md:w-1/2 aspect-[4/5] md:aspect-square relative overflow-hidden rounded-sm bg-slate-100 flex items-center justify-center shadow-[0_20px_50px_rgba(0,0,0,0.05)] group-hover:shadow-[0_30px_60px_rgba(124,58,237,0.15)] transition-shadow duration-700 ${index % 2 !== 0 ? 'md:order-2' : ''}`}>
                ...
                {project.image ? (
                  <img 
                    ...
                    className={`w-full h-full object-cover group-hover:scale-110 transition-transform duration-[1.5s] ease-[cubic-bezier(0.25,1,0.5,1)] ${
                      project.id === 'inforens' ? 'object-contain scale-90 group-hover:scale-100 drop-shadow-2xl' : ''
                    }`} 
                  />
                ) : (
                  <>
                    ...
                    <div className="relative z-10 text-slate-400 font-mono text-xs md:text-sm tracking-widest bg-white/80 backdrop-blur-md px-6 py-3 rounded-full border border-slate-200 shadow-sm">
                      {project.placeholder}
                    </div>
                  </>
                )}
                <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-500 z-20 pointer-events-none hidden md:flex">
              ...
              <div className="project-text w-full md:w-1/2 flex flex-col justify-center">
                <div className="flex items-center gap-4 mb-6 md:mb-8">
                  ...
                  <span className="text-[8px] md:text-[9px] font-mono text-slate-500 uppercase tracking-[0.2em] bg-slate-200/50 px-3 py-1 rounded-sm">
                    {project.tag}
                  </span>
                </div>
                
                <h2 className="font-poppins text-3xl md:text-5xl font-black text-brand-blue tracking-tighter mb-4 md:mb-6 group-hover:text-brand-accent-blue transition-colors duration-500">
                  {project.title}
                </h2>
                
                <h3 className="font-montserrat text-lg md:text-2xl text-brand-blue font-medium mb-4 md:mb-6 leading-tight">
                  {project.headline}
                </h3>
                
                <p className="font-montserrat text-slate-600 font-light mb-8 md:mb-10 leading-relaxed text-base md:text-lg">
                  {project.body}
                </p>
                
                <div className="bg-white/60 backdrop-blur-md p-6 md:p-8 rounded-sm border border-slate-200/80 mb-8 md:mb-10 shadow-[0_8px_30px_rgb(0,0,0,0.03)] group-hover:border-brand-accent-blue/20 transition-colors duration-500">
                  <p className="font-mono text-[9px] md:text-[10px] font-bold uppercase tracking-[0.2em] text-brand-accent-blue mb-4 md:mb-6">Project Scope</p>
                  <ul className="space-y-3 md:space-y-4 font-montserrat text-slate-600 text-xs md:text-sm font-medium">
                ...
                <Link to={`/case-study/${project.id}`} className="flex items-center gap-3 font-mono text-[10px] md:text-[11px] font-bold uppercase tracking-widest text-brand-blue w-max group-hover:text-brand-accent-blue transition-colors duration-300">
